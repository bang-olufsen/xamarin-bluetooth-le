...
namespace Plugin.BLE.Android
{
    public class Characteristic : CharacteristicBase<BluetoothGattCharacteristic>
    {
        ...
        protected override async Task<(byte[] data, int resultCode)> ReadNativeAsync()
        {
            ...
            {
                result = await TaskBuilder.FromEvent<(byte[] data, int resultCode), EventHandler<CharacteristicReadCallbackEventArgs>, EventHandler>(
        ...
        protected override async Task<int> WriteNativeAsync(byte[] data, CharacteristicWriteType writeType)
        {
            ...
            await CrazyQueue.Run(async () =>
            {
                result = await TaskBuilder.FromEvent<int, EventHandler<CharacteristicWriteCallbackEventArgs>, EventHandler>(
        ...
        protected override async Task StartUpdatesNativeAsync(CancellationToken token = default(CancellationToken))
        {
            ...
            if (NativeCharacteristic.Descriptors.Count > 0)
            {
                ...
                {
                    await descriptor.WriteAsync(BluetoothGattDescriptor.EnableIndicationValue.ToArray(), token);
                    Trace.Message("Descriptor set value: INDICATE");
                }

                if (descriptor != null && Properties.HasFlag(CharacteristicPropertyType.Notify))
                {
                    await descriptor.WriteAsync(BluetoothGattDescriptor.EnableNotificationValue.ToArray(), token);
